...
template <typename T>
void vector_subvec_memcpy(vector<T>* dst_vec, size_t dst_offset,
                          const vector<T>* src_vec, size_t src_offset,
                          size_t len) {
  cudaError_t err;
  if (dst_vec->stride == 1 && src_vec->stride == 1) {
    err = cudaMemcpy(reinterpret_cast<void*>(dst_vec->data + dst_offset),
